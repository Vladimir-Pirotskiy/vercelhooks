...
export class StblStaking implements Contract {
	...
	async getEscrowByStaker(
		...
		const escrow: Address = stack.readAddress();
		return escrow;
	}
	...
	async getNftCollectionAddress(provider: ContractProvider): Promise<Address> {
		...
		const stack = await data.stack;
		const collectionAddress: Address = stack.readAddress();
	...
	async getJettonAddresses(
		...
		jettonBWallet: Address;
	}> {
		const data = await provider.get("get_jetton_addresses", []);
		const stack = await data.stack;
		const jettonA = stack.readTuple();
		const jettonB = stack.readTuple();

		const jettonAMinter = jettonA.readAddress();
		const jettonAWallet = jettonA.readAddressOpt();

		const jettonBMinter = jettonB.readAddress();
		const jettonBWallet = jettonB.readAddressOpt();
	...
	async getPoolInfo(provider: ContractProvider): Promise<{
		...
		const stack = await data.stack;
		const set = stack.readBoolean();
		const apr = stack.readBigNumber();
		const reward_period = stack.readBigNumber();

		const jetton_b_balance = stack.readBigNumber();
		const jetton_b_allocated = stack.readBigNumber();
		const jetton_a_staked = stack.readBigNumber();
		const jetton_a_maximum_stake = stack.readBigNumber();
		const jetton_a_minimum_stake = stack.readBigNumber();
	...
	async getInitializer(provider: ContractProvider): Promise<Address> {
		const data = await provider.get("get_initializer", []);
		const stack = await data.stack;
		const initializer = stack.readAddress();
		return initializer;
	}
}
